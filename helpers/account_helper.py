        ...
        while token is None:
            ...
            if count == 5:
                raise AssertionError(
                    "Превышено количество попыток активационного токена!"
                )
            if token:
                return token
            time.sleep(1)
...
class AccountHelper:
    ...
    def change_password(
        ...
        response = self.dm_account_api.account_api.put_v1_account_password(
            json_data=json_data, validate_response=True
        )
        return response
        ...
        for item in response.json()["items"]:
            ...
            if user_login == login:
                ...
                if token:
                    ...
                    print(
                        f"ConfirmationLinkUri не найден в письме для пользователя {login}"
                    )
        if token is None:
